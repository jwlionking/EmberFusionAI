...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " efus-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  efus-tx [options] <hex-tx> [commands]  Update hex-encoded efus transaction\n" +
            "or:     efus-tx [options] -create [commands]   Create hex-encoded efus transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded efus transaction
