...
const char * const BITCOIN_CONF_FILENAME = "efus.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //efus-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EFUSCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EFUSCore
    // Mac: ~/Library/Application Support/EFUSCore
    // Unix: ~/.efuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EFUS";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EFUS";
#else
    // Unix
    return pathRet / ".efus";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty efus.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure EFUS Core copyright is not removed by accident
    if (copyright_devs.find("EFUS Core") == std::string::npos) {
