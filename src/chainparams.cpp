...
public:
    CMainParams() {
        ...
        consensus.nPowTargetTimespan = 9 * 60; // EFUS: 1 day
        consensus.nPowTargetSpacing = 3 * 60; // EFUS: 3 minutes
        consensus.nNewPowTargetSpacing = 90; // EFUS: 1.5 minutes
        ...
        vSporkAddresses = {"Eb5gxxgCEnABhpkR71phfxDYPPZZrgSo1u"};
        nMinSporkKeys = 1;
...
public:
    CTestNetParams() {
        ...
        consensus.powLimit = uint256S("00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 20
        consensus.nPowTargetTimespan = 24 * 60 * 60; // EFUS: 1 day
        consensus.nPowTargetSpacing = 2.5 * 60; // EFUS: 2.5 minutes
        ...
        vSeeds.emplace_back("testnet-seed.efusdot.io"); // Just a static list of stable node(s), only supports x9

        // Testnet EFUS addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Testnet EFUS script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Testnet private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Testnet EFUS BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet EFUS BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet EFUS BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("efusevo.org",  "devnet-seed.efusevo.org"));
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // Regtest EFUS addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest EFUS script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest EFUS BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest EFUS BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest EFUS BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
