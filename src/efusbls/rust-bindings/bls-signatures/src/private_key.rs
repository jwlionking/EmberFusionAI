...
impl PrivateKey {
    ...
    pub fn generate_efus(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "efus_helpers")]
    pub fn generate_efus_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_efus(rng))
            .collect()
    }
