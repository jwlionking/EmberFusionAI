...
The EFUS Core daemon can be configured to act as a trusted "border
router", implementing the efus wire protocol and relay, making
...
The ZeroMQ feature in EFUS Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building efusd:
...
For instance:
    ...
    $ efusd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/efusd.tx.raw \
...
These options can also be provided in efus.conf.
...
From the perspective of efusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into efusd directly. Furthermore, no information is
...
using. EFUSd appends an up-counting sequence number to each
