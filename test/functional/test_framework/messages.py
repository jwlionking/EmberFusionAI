...
# Objects that map to efusd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(efushash(r))
            self.hash = encode(efushash(r)[::-1], 'hex_codec').decode('ascii')
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(efushash(r))
            self.hash = int(encode(efushash(r)[::-1], 'hex_codec'), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in efusd indicates these should be deserialized as blocks
