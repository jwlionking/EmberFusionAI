...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave efusds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop efusds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use efus-cli instead of RPC for all commands")
        parser.add_argument("--efusd-arg", dest="efusd_extra_args", default=[], action="append",
                            help="Pass extra args to all efusd instances")
    ...
    def setup(self):
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/efusd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/efus-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.efusd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: efusds were not stopped and may still be running")
    ...
    def dynamically_initialize_datadir(self, chain, node_p2p_port, node_rpc_port):
        ...
        if chain == 'testnet3':
            ...
            chain_name_conf_arg_value = '1'
        with open(os.path.join(data_dir, "efus.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as efusd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in efus.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if efusd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("efusd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if efus-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("efus-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if efus-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("efus-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether efus-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether efus-wallet was compiled."""
...
class EFUSTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_efus_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, hpmn_count=0):
        ...
        self.set_efus_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in efus-q-recovery thread
    ...
    def set_efus_dip8_activation(self, activate_after_block):
    ...
    def set_efus_llmq_test_params(self, llmq_size, llmq_threshold):
